...
describe('Request', () => {
  it('Gets user data from the backend', () => {
    cy.request('POST', 'https://sauti-africa-market-staging-3.herokuapp.com/api/users', userId).should(
      response => {
...
describe('Request', () => {
  it('Gets user data from the backend', () => {
    cy.request('PUT', `https://sauti-africa-market-staging-3.herokuapp.com/api/users/google-oauth2|111661795839592168840`, role).should(
      response => {
        expect(response.status).to.eq(200)
        expect(response).to.have.property('headers')
        expect(response).to.have.property('duration')
